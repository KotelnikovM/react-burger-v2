...
const getTypeStr = (type: string | FlowType): string => {
  switch (type.name.toLowerCase()) {
    ...
    case 'enum':
      ...
      return type.value
        ? type.value.map((v: { value: string }) => `${v.value}`).join(' │ ')
        : type.raw
    case 'union':
      return type.value
        ? type.value.map((t: string) => `${getTypeStr(t)}`).join(' │ ')
        : type.raw
    case 'array':
      return type.raw
    case 'arrayof':
...
export const humanize = (type: string | FlowType) => getTypeStr(type)

export const getPropType = (prop: PropType) => {
  const propName: string = get('name', prop.flowType || prop.type)
